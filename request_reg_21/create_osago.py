...
import urllib
import json
from request_reg_21.json.save_policy import SavePolicyJson
from request_reg_21.json.save_voditel import SaveVoditelJson
from request_reg_21.json.save_contractor import SaveStrahJson, SaveSobstvJson
from request_reg_21.json.save_vehicle import SaveVehicleJson
from request_reg_21.json.get_contractor import GetContractorJson
from request_reg_21.json.check_policy_osago import CheckPolicyOsagoJson
from request_reg_21.json.osago_start_redirect import OsagoStartRedirectJson
import random
...
class CreateInsurance:
    ...
    def auth(self):
        self.r = self.session.get('https://eosago21-vek.ru/personal/?register=yes&backurl=%2Fpersonal%2Findex.php', headers=self.headers_site)
        ...
        self.r = self.session.post("https://eosago21-vek.ru/personal/index.php?login=yes", data=self.body, headers=self.headers_auth)
        print(self.r.status_code)

        self.r = self.session.post("https://eosago21-vek.ru/local/tools/webslon/elpolis.api/", json={"api_key": self.api_key, "method": "isAuthorized"})
        print(self.r.json())
    ...
    def create(self):
        ...
        if self.client_data.telephone == "" or \
            ...
            print(self.telephone.tel)
        else:
            self.tel_number = self.client_data.telephone

        # -------------------------------------------Страхователь-------------------------------------------------------
        self.document_strah = {
            "type": self.client_data.strah_type_document,
            "series": self.client_data.pass_seriya,
            "number": self.client_data.pass_number
        }
        self.s = SaveStrahJson(
            document=self.document_strah, name=self.client_data.name,
            ...
            telephone=self.tel_number, address=self.client_data.pass_address, api_key=self.api_key)
        ...
        while True:
            ...
            self.r = self.session.post("https://eosago21-vek.ru/local/tools/webslon/elpolis.api/", data=self.s(), headers=self.headers_create)
            print(self.r.json())
        ...
        if self.client_data.sobstv_yavl_strah != '"+"':
            self.document_sobstv = {
                "type": self.client_data.sobstv_type_document,
                "series": self.client_data.sobstv_pass_seriya,
                "number": self.client_data.sobstv_pass_number
            }
            self.s = SaveSobstvJson(
                document=self.document_sobstv,
                ...
                address=self.client_data.sobstv_pass_address, api_key=self.api_key)
        ...
        if self.client_data.c_ogr_or_not != "Без ограничений":
            ...
            for self.voditel_data in self.client_data.voditeli:
                ...
                self.s = SaveVoditelJson(name=self.voditel_data.name, middlename=self.voditel_data.otchestvo, lastname=self.voditel_data.surname,
                                    ...
                                    foreign_dl=self.voditel_data.foreign_dl, api_key=self.api_key, tr=self.voditel_data.tr)
        ...
        self.dict_args_vehicle["api_key"] = self.api_key
        ...
        #     self.r = self.session.post("https://eosago21-vek.ru/local/tools/webslon/elpolis.api/", data=self.s())
        #     print(self.r.json())
        ...
        self.batut()
        # ---------------------------------------Проверка ОСАГО---------------------------------------------------------
        self.s = CheckPolicyOsagoJson(draft_id=self.draft_id, api_key=self.api_key)
        self.start_while_true = datetime.now()
        while True:
            if (datetime.now() - self.start_while_true) > timedelta(minutes=4):
    ...
    def batut(self):
        ...
        while True:
            if (datetime.now() - self.start_while_true_batut) > timedelta(minutes=4):
                ...
                raise CreateError21
            self.s = OsagoStartRedirectJson(draft_id=self.draft_id, api_key=self.api_key)
            self.r = self.session.post("https://eosago21-vek.ru/local/tools/webslon/elpolis.api/", data=self.s(), headers=self.headers_create)
            print(self.r.json())
            if self.r.json().get("error") != 0:
...
class AuthError21(Exception):
    pass
class CreateError21(Exception):
    pass