...
import requests
import json
from request_reg_arm.json.save_policy import SavePolicyJson
from request_reg_arm.json.save_voditel import SaveVoditelJson
from request_reg_arm.json.save_contractor import SaveStrahJson, SaveSobstvJson
from request_reg_arm.json.save_vehicle import SaveVehicleJson
from request_reg_arm.json.get_contractor import GetContractorJson
from request_reg_arm.json.check_policy_osago import CheckPolicyOsagoJson
from request_reg_arm.json.osago_start_redirect import OsagoStartRedirectJson
import random
...
class CreateInsurance:
    ...
    def auth(self):
        ...
        self.r = self.session.post("https://b2c.armeec.ru/osago/policy/index.php?login=yes", data=self.body)
        print(self.r.status_code)

        self.r = self.session.post("https://b2c.armeec.ru/local/tools/webslon/elpolis.api/", json={"method":"isAuthorized"})
        print(self.r.json())
    ...
    def create(self):
        ...
        while True:
            ...
            self.r = self.session.post("https://b2c.armeec.ru/local/tools/webslon/elpolis.api/", data=self.s())
            print(self.r.json())
...
class AuthErrorArm(Exception):
    pass
class CreateErrorArm(Exception):
    pass