...
def start_find(input_url, client_data, telephone=None):
    client_data.set_value("status_bot", f'{datetime.now().strftime("%d.%m %H:%M")} Поиск лимитов')
    check_insurances = client_data.strah_comp
                        ...
                        if resp.status_code == 200:
                            if resp.json()['isChosen'] == False:
                                if check_contract_id_token(url_token, contract_id_token, comp_id):
                                    ...
                                    client_data.set_value("url_rca", input_url)
                                    return input_url
                            else:
                                print(f'Для ссылки {input_url} контракт зарегистрирован ранее')
                                raise AlreadyRegFailed
                    else:
                        print(f'СК: {comp_name} - свободных контрактов нет.')
        else:
            with suppress(Exception):
                print(f'{resp.json()[0]["message"]}')
            print(f'Прекращаю поиск. Требуется обновить входную ссылку.')
            raise NeededUpdateUrlRca
